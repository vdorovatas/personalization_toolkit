...
def setup_args():
    ...
    parser.add_argument('--no_grounding_sam', dest='grounding_sam', action='store_false', help='Disable grounding sam for mask extraction')
    parser.set_defaults(grounding_sam=True)
    ...
    for file_path in o:
        ...
        if args.dataset == 'MyVLM':
            if args.grounding_sam:
                instance_features = copy.deepcopy(instance['dino_obj_features']).reshape([-1, 1024])
            else:
...
def check_save(args,img_path):
    ...
    if args.variation == 'augment':
        expected = args.n_training_views * (args.n_augment + 1)
    else:
        expected = args.n_training_views
