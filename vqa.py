...
# IMPORTANT: all_obj_features is loaded in the order of my_objects (e.g.
# OBJECTS_THIS_IS_MY_MULTI), which differs from the alphabetical filesystem
# order in feature_objects. We must iterate over my_objects so that each
# object name is paired with the correct feature tensor.
object_to_features = {}
for idx, obj_name in enumerate(my_objects):
    if idx < len(all_obj_features) and all_obj_features[idx] is not None:
...
# Setup save path
multi_str = '_multi' if args.multi_concept else ''
n_views_str = f'{args.n_training_views}v' if args.n_training_views else 'allv'
run_name = f'{args.dataset}{multi_str}_{n_views_str}_{args.variation}'
save_path = os.path.join(args.save_folder, args.task, args.vlm_model, run_name)
