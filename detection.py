...
FIXED_THRESHOLDS = [0.75, 0.70, 0.65, 0.60]
...
def setup_args():
    ...
    parser.add_argument('--save_folder', type=str, default='results',
                        help='Directory to save detection result .npy files')
    ...
    # Set detection threshold based on mode if not explicitly provided
    if args.multi_concept and args.detect_thresh == 0.75:  # 0.75 is the default
        args.detect_thresh = 0.65
        print(f"✓ Multi-concept mode detected: using detection threshold {args.detect_thresh}")
...
def configure_environment(args) -> None:
    ...
    If CUDA_VISIBLE_DEVICES is already set (e.g., by SLURM), respect it
    unless --device_ids is explicitly provided.

    Args:
        args: Arguments object containing device_ids list
    """
    # If SLURM (or user) already set CUDA_VISIBLE_DEVICES and no explicit
    # --device_ids was passed, respect the existing assignment.
    slurm_assigned = os.environ.get("CUDA_VISIBLE_DEVICES")
    default_device_ids = [0]  # matches argparse default

    if hasattr(args, 'device_ids') and args.device_ids != default_device_ids:
        # Explicit --device_ids was provided, use it
        if isinstance(args.device_ids, str):
            device_ids = [x.strip() for x in args.device_ids.split(',') if x.strip()]
        else:
            device_ids = [str(x) for x in args.device_ids]
        device_ids_str = ','.join(device_ids)
        os.environ["CUDA_VISIBLE_DEVICES"] = device_ids_str
        print(f"✓ CUDA_VISIBLE_DEVICES set to: {device_ids_str}")
        print(f"✓ Using {len(device_ids)} GPU(s)")
    elif slurm_assigned:
        # SLURM or external scheduler set it, don't override
        device_ids = [x.strip() for x in slurm_assigned.split(',') if x.strip()]
        print(f"✓ CUDA_VISIBLE_DEVICES (from environment): {slurm_assigned}")
        print(f"✓ Using {len(device_ids)} GPU(s) (process-visible indices 0..{len(device_ids)-1})")
    else:
...
def setup_thresholds(args):
    ...
    # Use the detect_thresh value (which already handles multi-concept defaults)
    return [args.detect_thresh]
...
def process_single_image(args, data_batch, mask_extractor, all_obj_features, my_objects_raw):
    ...
    # print(f"   {image[0][:3, :3, 0]}")
    # print(f"   dino_image[0] top-left 3x3:")
    # print(f"   {dino_image[0][:3, :3, 0]}")
    # print("="*80 + "\n")
    ...
    for batch_idx in range(args.batch_size):
        ...
        # # --- DEBUG: First batch detailed analysis ---
        # if batch_idx == 0:
        #     print("\n" + "="*80)
        ...
        #     mask_feat = category_features_stacked[0].to(args.device)
        #     obj_feat = all_obj_features[0].to(args.device)
        ...
        #         mask_feat.unsqueeze(0).unsqueeze(0), obj_feat.unsqueeze(0), dim=2, eps=1e-8)
        ...
        #     for m_idx in range(min(3, len(category_features_stacked))):
        #         mask_feature = category_features_stacked[m_idx]
        #         sim = torch.nn.functional.cosine_similarity(
        #             mask_feature.to(args.device), all_obj_features.to(args.device), dim=2, eps=1e-8)
        #         sim[torch.isnan(sim) == True] = 0
        #         print(f"\n   Mask {m_idx}:")
        #         print(f"     sim shape: {sim.shape}")
        #         print(f"     sim max per object: {sim.max(dim=1)[0][:5].cpu().numpy()}")
...
def print_metrics(metrics, skipped_count, args):
    ...
    # For negative/fake splits, only report positive recall
    is_negative_split = (args.dataset == 'this-is-my' and
                         args.test_split is not None and
                         args.test_split.lower() in ['negative (other)', 'negative (hard)', 'fake'])

    if is_negative_split:
        print(f'\nAverage Metrics (INVERTED for negative/fake split):')
...
def save_results(metrics, args, threshold, object_names):
    """
    Save evaluation results to a single JSON file.

    Args:
        metrics: Dictionary of calculated metrics
        args: Configuration arguments
        threshold: Detection threshold used
        object_names: List of object/category names
    """
    results_dir = Path(args.save_folder)
    results_dir.mkdir(parents=True, exist_ok=True)

    mode_str = 'multi' if args.multi_concept else 'single'
    n_views = args.n_training_views if args.n_training_views else 'all'

    # Build filename
    if args.dataset == 'this-is-my' and args.test_split is not None:
        test_split_str = args.test_split.lower().replace(' ', '_').replace('(', '').replace(')', '')
        filename = f'{args.dataset}-{mode_str}-{test_split_str}-{threshold}-{n_views}v-{args.variation}.json'
    else:
        filename = f'{args.dataset}-{mode_str}-{threshold}-{n_views}v-{args.variation}.json'

    # Check if this is a negative/fake split
    is_negative_split = (args.dataset == 'this-is-my' and
                         args.test_split is not None and
                         args.test_split.lower() in ['negative (other)', 'negative (hard)', 'fake'])

    # Filter NaN for summary
    recall_clean = metrics['recall'][~np.isnan(metrics['recall'])]
    ...
    results = {
        "config": {
            ...
            "mode": mode_str,
            "threshold": threshold,
            "n_training_views": args.n_training_views,
            "variation": args.variation,
            "grounding_sam": args.grounding_sam,
        },
        "summary": {},
        "per_object": []
    }
    ...
    if is_negative_split:
        results["summary"]["recall"] = float(recall_clean.mean()) if len(recall_clean) > 0 else None
    else:
        precision_clean = metrics['precision'][~np.isnan(metrics['precision'])]
        precision_neg_clean = metrics['precision_neg'][~np.isnan(metrics['precision_neg'])]
        recall_neg_clean = metrics['recall_neg'][~np.isnan(metrics['recall_neg'])]
        results["summary"]["precision"] = float(precision_clean.mean()) if len(precision_clean) > 0 else None
        results["summary"]["neg_precision"] = float(precision_neg_clean.mean()) if len(precision_neg_clean) > 0 else None
        results["summary"]["recall"] = float(recall_clean.mean()) if len(recall_clean) > 0 else None
        results["summary"]["neg_recall"] = float(recall_neg_clean.mean()) if len(recall_neg_clean) > 0 else None

    if args.multi_concept:
        balanced_acc = (metrics['recall'] + metrics['recall_neg']) / 2
        balanced_acc_clean = balanced_acc[~np.isnan(balanced_acc)]
        results["summary"]["balanced_accuracy"] = float(balanced_acc_clean.mean()) if len(balanced_acc_clean) > 0 else None

    for idx, name in enumerate(object_names):
        if is_negative_split:
            entry = {
                "name": name,
                "recall": float(metrics['recall'][idx]) if not np.isnan(metrics['recall'][idx]) else None,
            }
        else:
            entry = {
                "name": name,
                "precision": float(metrics['precision'][idx]) if not np.isnan(metrics['precision'][idx]) else None,
                "recall": float(metrics['recall'][idx]) if not np.isnan(metrics['recall'][idx]) else None,
                "neg_precision": float(metrics['precision_neg'][idx]) if not np.isnan(metrics['precision_neg'][idx]) else None,
                "neg_recall": float(metrics['recall_neg'][idx]) if not np.isnan(metrics['recall_neg'][idx]) else None,
            }
            if args.multi_concept:
                acc = (metrics['recall'][idx] + metrics['recall_neg'][idx]) / 2
                entry["balanced_accuracy"] = float(acc) if not np.isnan(acc) else None
        results["per_object"].append(entry)
...
def print_per_object_metrics(display_names, metrics, args):
    ...
    invert_note = " (INVERTED)" if is_negative_split else ""
    ...
    for idx, name in enumerate(display_names):
        if args.multi_concept:
            ...
            acc = (metrics['recall'][idx] + metrics['recall_neg'][idx]) / 2
            if np.isnan(metrics['precision'][idx]) and np.isnan(metrics['recall'][idx]):
                continue
            print(f'{name}: '
                  ...
                  f'ACC {acc:.3f}')
        elif is_negative_split:
            if np.isnan(metrics['recall'][idx]):
                continue
            print(f'{name}: '
                  f'Recall {metrics["recall"][idx]:.3f}')
        else:
...
if __name__ == "__main__":
    ...
    # # --- DEBUG: GROUNDING DINO MODEL CHECK ---
    # import hashlib
    # print("\n" + "="*80)
    ...
    for threshold in thresholds:
        ...
        # Save results
        obj_names = display_names if args.multi_concept else my_objects
        save_results(metrics, args, threshold, obj_names)