...
from loader import get_dataloader, OBJECTS_YOLLAVA
from model import Train_Extractor

# Constants
PROGRESS_LOG_INTERVAL = 50

# Build short-name → category lookup for YoLLaVA.
# OBJECTS_YOLLAVA entries are like 'bo-dog', 'denisdang-face', 'toodles-galore-cartoon cat'.
# substring_before_last_dash gives the short name ('bo', 'denisdang', 'toodles-galore')
# and the part after the last dash is the category ('dog', 'face', 'cartoon cat').
YOLLAVA_CATEGORY_MAP = {}
for _obj in OBJECTS_YOLLAVA:
    _short = substring_before_last_dash(_obj)
    _category = _obj[len(_short) + 1:]  # part after the first dash following short name
    YOLLAVA_CATEGORY_MAP[_short] = _category
...
def process_labels(labels: List[str], dataset: str = '') -> List[str]:
    """Convert folder-name labels into G-DINO text prompts.

    For YoLLaVA, folder names are short instance IDs (e.g. 'bo', 'denisdang')
    that are meaningless to G-DINO. Use YOLLAVA_CATEGORY_MAP to look up the
    semantic category (e.g. 'dog', 'face').

    For this-is-my, rap, and similar datasets, folder names follow the
    'instance name-category' convention (e.g. 'Sherrys road bike-bike',
    'Anya-person'). Extract the category after the last dash.

    For myvlm (no dashes in folder names), rsplit returns the full label
    unchanged, which is already a meaningful noun.
    """
    if dataset == 'yollava':
        return [YOLLAVA_CATEGORY_MAP.get(label, label).replace('-', ' ') for label in labels]
...
def extract_batch_features(
    ...
    class_labels = process_labels(label, dataset=args.dataset)
