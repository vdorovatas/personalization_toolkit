...
class This_is_my_Dataset(BaseImageDataset):
    ...
    def __init__(self, args, transform, transform_dino, **kwargs):
        ...
        if args.split=="train":
            ...
            self.dataset = (
                ...
                glob.glob(os.path.join(data_path, "**", "*.webp"), recursive=True) +
                glob.glob(os.path.join(data_path, "**", "*.jpg"), recursive=True) +
                glob.glob(os.path.join(data_path, "**", "*.jpeg"), recursive=True)
            )
    ...
    def __getitem__(self, index: int) -> Tuple:
        ...
        """
        # Load image and ensure RGB (multi-concept cutouts may be RGBA)
